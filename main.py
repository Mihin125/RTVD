...
class VideoTracker(object):
    ...
    def run(self):
        ...
        while self.vdo.grab():
            ...
            if len(outputs) > 0:
                ...
                cls = outputs[:, -3]
                velocities_x = outputs[:, -2]
                velocities_y = outputs[:, -1]
                velocities = np.stack((velocities_x, velocities_y), axis = 1)
                img0 = draw_boxes(img0, bbox_xyxy, identities,cls, velocities)  # BGR
            ...
            if self.args.save_txt:
                with open(self.args.save_txt + str(idx_frame).zfill(4) + '.txt', 'a') as f:
                    for i in range(len(outputs)):
                        x1, y1, x2, y2,idx,s,v_x,v_y= outputs[i]
    ...
    def image_track(self, im0):
        ...
        if det is not None and len(det):  # det: (#obj, 6)  x1 y1 x2 y2 conf cls
            ...
            # print(cc)
            # ****************************** deepsort ****************************
            outputs = self.deepsort.update(bbox_xywh, confs,cc, im0)
...
if __name__ == '__main__':
    ...
    # args = parser.parse_args(['--classes', '0', '1', '2', '3'])
