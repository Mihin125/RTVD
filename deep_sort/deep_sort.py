...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences,s, ori_img):
        print("bb0",bbox_xywh)
        ...
        self.tracker.update(detections,s)
        ...
        for i,track in enumerate(self.tracker.tracks):
            ...
            if(self.tracker.prev_tracks.get(str(track_id))):
                prev_center = self.tracker.prev_tracks.get(str(track_id))
                velocity = np.array(center)-np.array(prev_center)
            self.tracker.prev_tracks[str(track_id)]= center
            cls=track.cls
            # cls=s.to(torch.int64)
            # cls = cls.numpy()
            # j=j+1
            # print(j)
            # if len(s)>j:
            ...
            # print("classes ",s)
            # print(j,cls)
            # print(np.array([x1,y1,x2,y2,track_id], dtype=int))
            print(velocity)
            outputs.append(np.array([x1,y1,x2,y2,track_id,cls,velocity[0],velocity[1]], dtype=int))
        if len(outputs) > 0:
